...
final class ExpandableTableView: UITableView {
    ...
    func registerTableViewCells() {
        self.registerNib(TeamHeaderTableViewCell.self)
        self.registerNib(TeamDetailTableViewCell.self)
    }
...
extension ExpandableTableView: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if items[indexPath.row] is Header {
            let cell = tableView.dequeueCell(TeamHeaderTableViewCell.self, for: indexPath)
            return cell
        } else if items[indexPath.row] is Detail {
            let cell = tableView.dequeueCell(TeamDetailTableViewCell.self, for: indexPath)
            return cell
        }
